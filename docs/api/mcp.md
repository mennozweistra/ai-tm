...
| A budgeted phase's pass found a task failed with no fix applied, or exhausted `max_attempts` after applying a fix | Ticket run stopped; stop line names the phase; final report emitted. |
| A budgeted task's run failed with no fix applied, or exhausted `max_attempts` after applying a fix | Ticket run stopped; stop line names the task; final report emitted. |
...
**Exhausted budget stays exhausted.** Once a budgeted phase or task is marked `failed` after exhausting its budget, a later `tm_grind` call on the same ticket reads as a resume (§14.2) and does not reset its `attempts` — the human must clear it explicitly (`tm_phase_set`/`tm_task_set` with `attempts: 0`) before re-grinding.
