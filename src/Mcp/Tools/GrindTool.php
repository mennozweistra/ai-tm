...
5. **Subagent prompt template.** Replace every angle-bracketed placeholder with the actual value from the tm response.
   ...
   If what you are checking is broken, weigh the finding against the repair threshold before touching anything. Repair only a finding that would change what gets built or executed: a wrong name or path an implementer would build from, a contradiction between two texts, a step that cannot work as written, a failing gate. A finding below that bar — wording, phrasing, completeness polish, a sharper formulation that changes no behavior — is not repaired: record it in your summary (or as a question if it deserves the user's eye); it does not count as a fix and never triggers a re-run. If <round> is greater than 1, your scope is narrower still: confirm the previous round's repairs, nothing else. Read the previous round's `code_fix`/`assumption` log entries on this task via `tm_log_list`, verify each recorded repair actually holds, and repair only a repair that is itself wrong. Do not re-review the rest of the surface: a new finding outside those repairs — whatever its size — is stored as a question (`tm_question_add`, kind `check` or `ask`) and never repaired in this run. If this is the final allowed attempt (<round> equals the max in the CHECK BUDGET line) and you still apply a repair, additionally store a `check` question naming that repair as unconfirmed — the budget leaves no confirming run — and begin your summary with `REVIEW:` instead of `OK:`. A repair that passes the threshold is made inline in this same run: find the root cause and make it. A "repair" is whatever fixes this check's own concern, and it takes one of two forms. For a check whose subject is code, the repair is a code change: make it, run the project's quality gate, and commit it as one commit kept separate from any other commit this run makes. For a check whose subject is `tm` data rather than code — for example a Devil's Advocate or grind-compatibility review that repairs a requirement, a task description, or a phase through `tm` MCP calls — the repair is that data edit; there is no code commit and no quality gate to run for it. Either way, record it: call `mcp__tm__tm_log_add` with `task: <task.id>`, `type: "code_fix"` or `"assumption"`, `title`: a short one-line summary of the repair, and `ai_content`: the finding verbatim (what was broken), the repair you made, the commit id (or `none` for a data-only repair), who registered it (name yourself as the subagent, and the model you ran as), and the round number (<round>). Any real repair — code or `tm` data — means you report `fix applied: yes` (see OUTPUT FORMAT). If the check already passes, or every finding fell below the repair threshold and you repaired nothing, do not log a code_fix entry and report `fix applied: no`.
   ...
   The `fix applied:` line is machine-read by the coordinator (§7b/§8b) — it must be exactly `fix applied: yes` or `fix applied: no`, on its own line, nothing else on that line. It reports what you did, not what git shows: `yes` when you made ANY real repair this run — a code change you committed, OR a `tm` data change (a requirement rewritten, a task description corrected, a phase adjusted) — anything beyond just judging and finding nothing wrong; `no` when the run repaired nothing — it found nothing wrong, or every finding stayed below the CHECK BUDGET block's repair threshold and was only recorded. A `tm`-data repair with no code commit is still `fix applied: yes`. Every CHECK subagent MUST emit this line.
...
6. **Final report — a helicopter view, not a walk of the parts (req 376).** After every ticket has been processed (or the loop was aborted), output one block to chat. Report at the top level how the run went, then per-ticket counts. Do NOT list the questions or enumerate individual tasks — the report gives numbers; the stored detail lives in the dashboard and in the questions table.
   ...
   **On-demand listing (req 366).** The report shows counts only. When the user then asks to see the questions, call `mcp__tm__tm_question_list` for the ticket and present each stored question exactly as it was written — numbered so the user can refer to items, each item self-contained with its recommendation. Never summarize, rephrase, or reconstruct a question from a task result or from memory; replay the stored `question` and `recommendation` text verbatim.
   ...
   If any budgeted phase or budgeted task in this ticket hit the terminal case in §7d or §8d, append a line to that ticket's block: `  phase <id> stopped: failed with no fix applied` (or the `task <id>` equivalent). If any budget was exhausted with a fix still applied (the run continued, req 770), append: `  phase <id> budget exhausted (<n>) — last repair unconfirmed, see its check question` (or the `task <id>` equivalent).
...
7. **Running a budgeted phase (check-phase passes).**
   ...
   d. **Decide, using the results collected in step b.** (A phase's `status` is not something the coordinator sets directly: under `auto_status`, `ai-lib` derives it automatically from its tasks' statuses — once every task in a pass is `done`, the phase already reads `done`; once a task is `failed`, the phase already reads `failed`. Only `attempts`, in step c, is written explicitly.)
      ...
      - **At least one task reported `fix applied: yes`:** the phase must re-run — a check that just fixed itself is unverified until a subsequent pass confirms it clean, even if every task in this pass happens to read `done` right now. Check exhaustion first, using the `attempts` value just written in step c: if `attempts >= max_attempts`, the budget is spent before that confirming pass could run. Do not stop the ticket and do not mark anything `failed` (req 770): the final pass's repairing subagents each stored an unconfirmed-repair `check` question and returned `REVIEW:` per §5, so their tasks read `review`; if any task that reported `fix applied: yes` in this final pass nevertheless ended `done`, set it to `review` via `mcp__tm__tm_task_set` so the unconfirmed repair stays visible. Note the exhaustion (phase id, attempts) for the final report (§6) and move to the next phase. Otherwise (budget remains), go back to step a and run another pass.
...
8. **Running a budgeted task outside a check-phase (self-contained check, req 295).**
   ...
   d. **Decide:**
      ...
      - **A fix was applied:** the task must re-run — a check that just fixed itself is unverified until a subsequent run confirms it clean, even though this run's own token may have been `OK:` (status `done`). Check exhaustion first using the `attempts` value just written in step c: if `attempts >= max_attempts`, the budget is spent before that confirming run could happen. Do not stop the ticket and do not mark the task `failed` (req 770): the final run stored an unconfirmed-repair `check` question and returned `REVIEW:` per §5; if it nevertheless ended `done`, override it to `review` (`mcp__tm__tm_task_set`, `status: "review"`) so the unconfirmed repair stays visible. Note the exhaustion for the final report (§6) and continue the task walk in §3d with the next task in the phase. Otherwise (budget remains), reset just this task to `pending` (`mcp__tm__tm_task_set`, `status: "pending"` — its phase is untouched) and go back to step a for another run.

   **Exhaustion and later grinds (resolved under ticket 301, req 770):** an exhausted budget no longer marks anything `failed`, so it can no longer strand a ticket: the exhausted check's tasks read `done` or `review`, §3d and §7's skip-if-done pass over them on a resume, and a fresh grind (§3c) resets `attempts` to 0 as always. To deliberately re-run an exhausted check, the human resets the task or phase to `pending` and its `attempts` to 0 by hand.
...
4. A budgeted phase's pass applied zero fixes while a task still failed (§7d). Output the stop line from §7d, then the final report (§6), and stop.
5. A budgeted task outside a check-phase failed with no fix applied (§8d). Output the stop line from §8d, then the final report (§6), and stop.

Budget exhaustion is deliberately not on this list (req 770): a spent budget with a fix still applied leaves the check in `review` with its unconfirmed-repair question stored, and the run continues (§7d/§8d).
