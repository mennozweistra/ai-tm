<?php

declare(strict_types=1);

namespace AiToolset\Tm\Cli\Commands;

use AiToolset\Tm\Cli\BaseCommand;
use AiToolset\AiLib\Services\TaskService;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Input\InputOption;
use Symfony\Component\Console\Output\OutputInterface;

final class TaskSetCommand extends BaseCommand
{
    public function __construct(
        private readonly TaskService $service,
    ) {
        parent::__construct();
    }

    protected function configure(): void
    {
        $this->setName('task:set')
            ->addOption('task', null, InputOption::VALUE_REQUIRED)
            ->addOption('name', null, InputOption::VALUE_OPTIONAL)
            ->addOption('description', null, InputOption::VALUE_OPTIONAL)
            ->addOption('ai-description', null, InputOption::VALUE_OPTIONAL)
            ->addOption('result', null, InputOption::VALUE_OPTIONAL)
            ->addOption('ai-result', null, InputOption::VALUE_OPTIONAL)
            ->addOption('status', null, InputOption::VALUE_OPTIONAL)
            ->addOption('max-attempts', null, InputOption::VALUE_OPTIONAL)
            ->addOption('attempts', null, InputOption::VALUE_OPTIONAL)
            ->addOption('actor', null, InputOption::VALUE_OPTIONAL)
            ->addOption('model', null, InputOption::VALUE_OPTIONAL)
            ->addOption('phase', null, InputOption::VALUE_OPTIONAL);
    }

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        try {
            $taskId = $this->requireIntOption($input, 'task');
            $newPhaseId = $this->optionalIntOption($input, 'phase');

            if ($newPhaseId !== null) {
                $this->service->reassign($taskId, $newPhaseId);
            }

            $hasSetFields = $input->getOption('name') !== null
                || $input->getOption('description') !== null
                || $input->getOption('ai-description') !== null
                || $input->getOption('result') !== null
                || $input->getOption('ai-result') !== null
                || $input->getOption('status') !== null
                || $input->getOption('max-attempts') !== null
                || $input->getOption('attempts') !== null
                || $input->getOption('actor') !== null
                || $input->getOption('model') !== null;

            if ($hasSetFields || $newPhaseId === null) {
                $out = $this->service->set(
                    id: $taskId,
                    name: is_string($input->getOption('name')) ? $input->getOption('name') : null,
                    description: is_string($input->getOption('description')) ? $input->getOption('description') : null,
                    aiDescription: is_string($input->getOption('ai-description')) ? $input->getOption('ai-description') : null,
                    status: is_string($input->getOption('status')) ? $input->getOption('status') : null,
                    result: is_string($input->getOption('result')) ? $input->getOption('result') : null,
                    aiResult: is_string($input->getOption('ai-result')) ? $input->getOption('ai-result') : null,
                    actor: is_string($input->getOption('actor')) ? $input->getOption('actor') : null,
                    model: is_string($input->getOption('model')) ? $input->getOption('model') : null,
                    maxAttempts: $this->optionalIntOption($input, 'max-attempts'),
                    attempts: $this->optionalIntOption($input, 'attempts'),
                );
            } else {
                $out = $this->service->show($taskId);
            }

            return $this->success($output, $out);
        } catch (\Throwable $e) {
            return $this->handleError($output, $e);
        }
    }
}
